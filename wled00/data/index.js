...
function openTab(tabI, force = false)
{
	...
	d.body.classList.toggle('hm-mapper-focus', tabI == 4);
	hmSetLive(tabI == 4);
	iSlide = tabI;
	...
	switch (tabI) {
		...
		case 4: window.location.hash = "Layout"; hmInit(); hmSetLive(true); break;
	}
}
...
function makeWS() {
	...
	ws.onmessage = (e)=>{
		if (e.data instanceof ArrayBuffer) { hmHandleLiveFrame(e.data); return; } // liveview packet
		var json = JSON.parse(e.data);
		if (json.leds) { hmHandleLiveJson(json.leds); return; } // JSON liveview packet
...
function togglePcMode(fromB = false)
{
	...
	d.body.classList.toggle('hm-pc', pcMode);
	...
	if (!pcMode) d.body.classList.remove('hm-mapper-focus');
	gId('buttonPcm').className = (pcMode) ? "active":"";
	gId('bot').style.height = (pcMode && !cfg.comp.pcmbot) ? "0":"auto";
	sCol('--bh', gId('bot').clientHeight + "px");
	_C.style.width = (pcMode || simplifiedUI)?'100%':'500%';
	_C.style.setProperty('--n', pcMode ? 4 : N);
}
...
function hmSetTool(t) { hmTool=t; ["point","line","select"].forEach(x=>hmEl("tool-"+x).classList.toggle("active",x==t)); }
function hmSetOutput(v) { hmCurrentOutput=parseInt(v,10)||0; hmDrawCount(); }
function hmRgb(hex) { hex=hex.replace("#",""); return [0,2,4].map(i=>parseInt(hex.slice(i,i+2),16)); }
function hmLiveColor(id, fallback="#ffd166") { return hmLiveColors[id] || fallback; }
function hmApplyLiveColors() {
	hmPixels.forEach(p=>{
		let el=d.querySelector(`#hm-pixels .hm-pixel[data-id="${p.id}"]`);
		if(el)el.style.setProperty("--c",hmLiveColor(p.id,p.color||"#ffd166"));
	});
	d.querySelectorAll("#hm-wires line[data-to]").forEach(l=>l.setAttribute("stroke",hmLiveColor(l.dataset.to,"rgba(255,255,255,.42)")));
}
function hmHandleLiveJson(leds) {
	if(!hmLiveOn||!Array.isArray(leds))return;
	hmLiveColors=leds.map(c=>"#"+((c.length>6)?c.substring(2):c));
	hmApplyLiveColors();
}
function hmHandleLiveFrame(data) {
	if(!hmLiveOn)return;
	let leds=new Uint8Array(data);
	if(leds[0]!=76)return;
	let start=leds[1]==2?4:2, colors=[];
	for(let i=start,id=0;i<leds.length;i+=3,id++) colors[id]=`rgb(${leds[i]},${leds[i+1]},${leds[i+2]})`;
	hmLiveColors=colors;
	hmApplyLiveColors();
}
function hmPollLive(retry=0) {
	clearTimeout(hmLiveTimer);
	if(!hmLiveOn)return;
	fetch(getURL("/json/live")).then(r=>r.ok?r.json():Promise.reject()).then(j=>{hmHandleLiveJson(j.leds);hmLiveTimer=setTimeout(hmPollLive,80);}).catch(()=>{if(hmLiveOn)hmLiveTimer=setTimeout(()=>hmPollLive(retry+1),retry<5?1000:2500);});
}
function hmSetLive(on) {
	hmLiveOn=on;
	clearTimeout(hmLiveTimer);
	if(!on){if(ws&&ws.readyState===WebSocket.OPEN)ws.send('{"lv":false}');return;}
	if(ws&&ws.readyState===WebSocket.OPEN){ws.send('{"lv":true}');return;}
	hmPollLive();
}
function hmBuildOutputs() {
	let ins=hmCfg&&hmCfg.hw&&hmCfg.hw.led&&hmCfg.hw.led.ins;
	if (Array.isArray(ins)&&ins.length) hmOutputs=ins.map((o,i)=>({id:i,name:"Output "+(i+1),start:o.start||0,len:o.len||0,pin:Array.isArray(o.pin)?o.pin.join(","):o.pin}));
	else hmOutputs=[{id:0,name:"Output 1",start:0,len:(lastinfo.leds&&lastinfo.leds.count)||0}];
	let s=hmEl("outputSel");
	if (s) {
		s.innerHTML=hmOutputs.map(o=>`<option value="${o.id}">${o.name}${o.pin!=null?" - pin "+o.pin:""} (${o.len||"?"})</option>`).join("");
		s.value=hmCurrentOutput;
	}
}
function hmNextId() { return hmPixels.length?Math.max(...hmPixels.map(p=>p.id))+1:0; }
function hmRenumberPixels() { hmPixels.sort((a,b)=>a.id-b.id).forEach((p,i)=>p.id=i); hmSegments=[]; hmActiveSegment=null; }
function hmIdsForRun(id) { let p=hmPixels.find(x=>x.id==id); return p&&p.run&&p.run.startsWith("run-")?hmPixels.filter(x=>x.run==p.run).map(x=>x.id):[id]; }
function hmLineEndDrag(id) {
	let ids=hmIdsForRun(id).sort((a,b)=>a-b);
	if(ids.length<2||!ids.includes(id)||(id!=ids[0]&&id!=ids[ids.length-1]))return null;
	let anchorId=id==ids[0]?ids[ids.length-1]:ids[0],anchor=hmPixels.find(p=>p.id==anchorId);
	return anchor?{mode:"line-end",id,ids,anchor:{x:anchor.x,y:anchor.y},moveFirst:id==ids[0],x:0,y:0,moved:false}:null;
}
function hmOrderedIdsFrom(id) { let ids=hmIdsForRun(id).sort((a,b)=>a-b),at=ids.indexOf(id); if(at==ids.length-1)return ids.reverse(); return at>0?ids.slice(at).concat(ids.slice(0,at)):ids; }
...
function hmLinePointsN(a,b,n) { let out=[]; if(n<2)return[{x:a.x,y:a.y}]; for(let i=0;i<n;i++){let f=i/(n-1);out.push({x:a.x+(b.x-a.x)*f,y:a.y+(b.y-a.y)*f});} return out; }
...
function hmUpdatePictureRect() {
	...
	if(nw&&nh&&!bg.classList.contains("hm-hidden")){let s=Math.min(r.width/nw,r.height/nh);hmPic={w:nw*s,h:nh*s};hmPic.x=(r.width-hmPic.w)/2;hmPic.y=(r.height-hmPic.h)/2;} else hmPic={x:0,y:0,w:r.width,h:r.height};
	canvas.style.left=hmPic.x+"px"; canvas.style.top=hmPic.y+"px"; canvas.style.width=hmPic.w+"px"; canvas.style.height=hmPic.h+"px"; hmApplyView();
}
function hmApplyView() { let c=hmEl("canvas"); c.style.setProperty("--z",hmZoom); c.style.transform=`translate(${hmPanX}px,${hmPanY}px) scale(${hmZoom})`; }
function hmSetZoom(v,cx,cy) { let old=hmZoom; cx=cx??(hmPic.x+hmPic.w/2); cy=cy??(hmPic.y+hmPic.h/2); hmZoom=Math.max(1,Math.min(6,parseFloat(v)/100)); let lx=(cx-hmPic.x-hmPanX)/old,ly=(cy-hmPic.y-hmPanY)/old; hmPanX=cx-hmPic.x-lx*hmZoom; hmPanY=cy-hmPic.y-ly*hmZoom; hmApplyView(); }
function hmResetView() { hmZoom=1; hmPanX=0; hmPanY=0; hmApplyView(); }
function hmPt(e) { let r=hmStageRect(),x=(e.clientX-r.left-hmPic.x-hmPanX)/hmZoom,y=(e.clientY-r.top-hmPic.y-hmPanY)/hmZoom; return {x:Math.max(0,Math.min(100,x/hmPic.w*100)),y:Math.max(0,Math.min(100,y/hmPic.h*100))}; }
function hmClearPreview() { hmEl("linePreview").innerHTML=""; hmLineEnd=null; }
function hmDrawPreview() { if(!hmLineStart||!hmLineEnd)return; hmEl("linePreview").innerHTML=hmLinePoints(hmLineStart,hmLineEnd).map(p=>`<span class=hm-pixel style="left:${p.x}%;top:${p.y}%"></span>`).join(""); }
function hmStageDown(e) { hmHideMenu(); if(e.target.dataset.id)return; hmEl("stage").setPointerCapture&&hmEl("stage").setPointerCapture(e.pointerId); let p=hmPt(e); if(hmTool=="line"){hmLineStart=p;hmLineEnd=p;hmDrawPreview();return;} hmPanDrag={x:e.clientX,y:e.clientY,px:hmPanX,py:hmPanY,p,tool:hmTool,moved:false}; }
function hmStageMove(e) {
	if(hmPinch)return;
	if(e.pointerType=="mouse"&&e.buttons===0&&(hmPanDrag||hmDrag||hmLineStart)){hmStageUp(e);return;}
	if(hmPanDrag){let dx=e.clientX-hmPanDrag.x,dy=e.clientY-hmPanDrag.y;if(Math.hypot(dx,dy)>3)hmPanDrag.moved=true;if(hmPanDrag.moved){hmPanX=hmPanDrag.px+dx;hmPanY=hmPanDrag.py+dy;hmApplyView();}return;}
	if(hmLineStart&&hmTool=="line"){hmLineEnd=hmPt(e);hmDrawPreview();return;}
	if(!hmDrag)return;
	if(!hmDrag.moved&&Math.hypot(e.clientX-hmDrag.x,e.clientY-hmDrag.y)>3){hmDrag.moved=true;clearTimeout(hmPressTimer);hmHideMenu();}
	if(hmDrag.mode=="line-end"){let p=hmPt(e),pts=hmDrag.moveFirst?hmLinePointsN(p,hmDrag.anchor,hmDrag.ids.length):hmLinePointsN(hmDrag.anchor,p,hmDrag.ids.length);hmDrag.ids.forEach((id,i)=>{let px=hmPixels.find(x=>x.id==id);if(px){px.x=pts[i].x;px.y=pts[i].y;}});hmDraw();return;}
	let p=hmPt(e); hmPixels.forEach(px=>{let o=hmDrag.offsets&&hmDrag.offsets[px.id]; if(o){px.x=p.x-o.dx;px.y=p.y-o.dy;}}); hmDraw();
}
...
function hmPixelDown(e,id) {
	...
	hmDrag=hmLineEndDrag(id)||{id,offsets,moved:false}; hmDrag.x=e.clientX; hmDrag.y=e.clientY; hmPressTimer=setTimeout(()=>{if(hmDrag&&!hmDrag.moved)hmShowMenu(e.clientX,e.clientY,id);},550); hmDraw();
}
function hmShowMenu(x,y,id) { hmMenuPixel=id; if(!hmSelected.includes(id))hmSelected=hmIdsForRun(id); let m=hmEl("menu"); m.style.left=x+"px"; m.style.top=y+"px"; m.classList.remove("hm-hidden"); hmDraw(); }
function hmHideMenu() { hmEl("menu").classList.add("hm-hidden"); }
function hmDraw() {
	let pxWrap=hmEl("pixels"), wires=hmEl("wires"); pxWrap.innerHTML=""; wires.innerHTML="";
	hmPixels.sort((a,b)=>a.id-b.id).forEach((p,i)=>{if(i){let q=hmPixels[i-1];if((q.out||0)==(p.out||0)){let l=document.createElementNS("http://www.w3.org/2000/svg","line");l.dataset.to=p.id;l.setAttribute("x1",q.x+"%");l.setAttribute("y1",q.y+"%");l.setAttribute("x2",p.x+"%");l.setAttribute("y2",p.y+"%");l.setAttribute("stroke",hmLiveColor(p.id,"rgba(255,255,255,.42)"));wires.appendChild(l);}}let b=document.createElement("span");b.className="hm-pixel"+(hmSelected.includes(p.id)?" sel":"");b.dataset.id=p.id;b.role="button";b.tabIndex=0;b.style.left=p.x+"%";b.style.top=p.y+"%";b.style.setProperty("--c",hmLiveColor(p.id,p.color||"#ffd166"));b.title="Pixel "+p.id+" - Output "+((p.out||0)+1)+(p.run&&p.run.startsWith("run-")?" (line)":"");b.onpointerdown=e=>hmPixelDown(e,p.id);b.oncontextmenu=e=>{e.preventDefault();hmShowMenu(e.clientX,e.clientY,p.id);};pxWrap.appendChild(b);});
	hmDrawCount(); hmDrawSegments();
}
...
function hmDeleteSelected(){let del=[...hmSelected];if(hmMenuPixel!=null)del=[...new Set(del.concat(hmIdsForRun(hmMenuPixel)))];hmPixels=hmPixels.filter(p=>!del.includes(p.id));hmSelected=[];hmHideMenu();hmRenumberPixels();hmDraw();}
...
function hmSetRunStart(){if(hmMenuPixel==null)return;let ids=hmOrderedIdsFrom(hmMenuPixel),old=Math.min(...ids),v=prompt("Start pixel number",old);if(v==null)return;let n=parseInt(v,10);if(isNaN(n)){hmEl("msg").textContent="Enter a pixel number.";return;}hmReorderIds(ids,n);hmHideMenu();hmDraw();hmEl("msg").textContent="Line starts at pixel "+Math.max(0,Math.min(n,hmPixels.length-ids.length))+" from the dot you picked.";}
function hmReverseRun(){if(hmMenuPixel==null)return;let ids=hmIdsForRun(hmMenuPixel).sort((a,b)=>a-b).reverse(),start=Math.min(...ids);hmReorderIds(ids,start);hmHideMenu();hmDraw();hmEl("msg").textContent="Line direction reversed.";}
...
function hmDeleteKey(e){let tag=(e.target&&e.target.tagName||"").toLowerCase();if(tag=="input"||tag=="textarea"||tag=="select"||!hmEl("stage")||(!d.body.classList.contains("hm-mapper-focus")&&iSlide!=4))return;if(e.key!="Delete"&&e.key!="Backspace")return;if(hmActiveSegment!=null&&hmSegments[hmActiveSegment]){e.preventDefault();hmDeleteSegment();return;}if(hmSelected.length){e.preventDefault();hmMenuPixel=null;hmDeleteSelected();}}
