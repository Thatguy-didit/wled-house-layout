...
function hmStageUp(e) {
	...
	if(hmPanDrag){let pd=hmPanDrag;hmPanDrag=null;if(!pd.moved){if(pd.tool=="point")hmAddPixel(pd.p.x,pd.p.y);else if(pd.tool=="select"){hmSelected=[];hmDraw();}}return;}
...
function hmPixelDown(e,id) {
	...
	hmDrag={id,offsets,x:e.clientX,y:e.clientY,moved:false}; hmPressTimer=setTimeout(()=>{if(hmDrag&&!hmDrag.moved)hmShowMenu(e.clientX,e.clientY,id);},550); hmDraw();
}
